...
USER_HOPPUS_DIR:=user/hoppus/src
USER_HOPPUS_INCLUDE_DIR:=user/hoppus/include
...
CFLAGS += -I$(USER_HOPPUS_INCLUDE_DIR)
CFLAGS += $(CFLAGS_MISC_DEFINE)
$(info $$HOPPUS_INCLUDE is [${USER_HOPPUS_INCLUDE_DIR}])
...
USER_HOPPUS_SOURCES:=$(shell find $(USER_HOPPUS_DIR) -name "*.c")
...
USER_HOPPUS_SOURCE_OBJECTS = $(USER_HOPPUS_SOURCES:$(USER_HOPPUS_DIR)/%.c=$(BUILD_DIR)/%.o)
...
$(info $$USER_HOPPUS_SOURCE_OBJECTS is [${USER_HOPPUS_SOURCE_OBJECTS}])
...
$(USER_SOURCE_OBJECTS) : $$(patsubst $(BUILD_DIR)/%.o,$(USER_DIR)/%.c,$$@)
	mkdir -p $(@D)
	$(CC) -c -o $@ $(CFLAGS) $<
$(USER_HOPPUS_SOURCE_OBJECTS) : $$(patsubst $(BUILD_DIR)/%.o,$(USER_HOPPUS_DIR)/%.c,$$@)
	mkdir -p $(@D)
	$(CC) -c -o $@ $(CFLAGS) $<

$(TARGET): $(KERNEL_SOURCE_OBJECTS) $(ASSEMBLY_OBJECTS) $(PLATFORM_SOURCE_OBJECTS) $(USER_SOURCE_OBJECTS) $(USER_HOPPUS_SOURCE_OBJECTS) $(LINKERSCRIPT) $(INCLUDES_DIR) $(USER_INCLUDES_DIR) $(USER_HOPPUS_INCLUDE_DIR)
